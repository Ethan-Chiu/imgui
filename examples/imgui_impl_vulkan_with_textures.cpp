...
struct ImGui_ImplVulkan_Data
{
    ...
    // Changed: VkDescriptorSet             DescriptorSet;
    VkPipeline                  Pipeline;
    uint32_t                    Subpass;
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkanH_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    ...
    // Bind pipeline and descriptor sets:
    {
        ...
        // Changed: VkDescriptorSet desc_set[1] = { bd->DescriptorSet };
        // Changed: vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, desc_set, 0, NULL);
    }
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    // Changed: {
    // Changed:     VkDescriptorImageInfo desc_image[1] = {};
    // Changed:     desc_image[0].sampler = bd->FontSampler;
    // Changed:     desc_image[0].imageView = bd->FontView;
    // Changed:     desc_image[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    // Changed:     VkWriteDescriptorSet write_desc[1] = {};
    // Changed:     write_desc[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    // Changed:     write_desc[0].dstSet = bd->DescriptorSet;
    // Changed:     write_desc[0].descriptorCount = 1;
    // Changed:     write_desc[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    // Changed:     write_desc[0].pImageInfo = desc_image;
    // Changed:     vkUpdateDescriptorSets(v->Device, 1, write_desc, 0, NULL);
    // Changed: }
    // Added by Ethane
    VkDescriptorSet fontDescriptorSet = (VkDescriptorSet)ImGui_ImplVulkan_AddTexture_Internal(bd->FontSampler, bd->FontView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    // Changed: io.Fonts->SetTexID((ImTextureID)(intptr_t)bd->FontImage);
    // Added by Ethane
...
static void ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ...
    // Changed: binding[0].pImmutableSamplers = sampler;
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    // Changed: {
    // Changed:     VkDescriptorSetAllocateInfo alloc_info = {};
    // Changed:     alloc_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    // Changed:     alloc_info.descriptorPool = v->DescriptorPool;
    // Changed:     alloc_info.descriptorSetCount = 1;
    // Changed:     alloc_info.pSetLayouts = &bd->DescriptorSetLayout;
    // Changed:     err = vkAllocateDescriptorSets(v->Device, &alloc_info, &bd->DescriptorSet);
    // Changed:     check_vk_result(err);
    // Changed: }
